...
public class LoginResource {
  ...
  public void login(@NotNull @Context final HttpServletResponse response,
      @ApiParam(name = "callback",
          value = "URL to send the user back to after authentication") @RequestParam(
              name = Constants.CALLBACK_PARAM, required = false) String callback,
      @ApiParam(name = "sp_id", value = "Service provider id") @RequestParam(name = "sp_id",
          required = false) String spId)
      throws Exception {
    String accessCode = loginService.issueAccessCode(spId);

    String realCallback = callback;
    if (StringUtils.isBlank(realCallback)) {
      realCallback = properties.getHomePageUrl();
    } else {
      whiteList.validate("callback", realCallback);
    }

    response.sendRedirect(addAccessCode(realCallback, accessCode));
  }
  ...
  protected String addAccessCode(String callback, String accessCode) throws Exception {
    return new URIBuilder(callback).setParameter("accessCode", accessCode).build().toString();
  }
